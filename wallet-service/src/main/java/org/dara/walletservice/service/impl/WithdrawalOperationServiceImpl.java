...
public class WithdrawalOperationServiceImpl implements WithdrawalOperationService {
    ...
    @Transactional
    @Override
    public WithdrawalResponse process(UUID userUuid, WithdrawalRequest request, String idempotencyKey) {

        Wallet wallet = walletRepository.findByUserUuid(userUuid).orElseThrow(() -> new WalletBalanceNotFoundException("Wallet not found"));

        Asset asset = assetRepository.findBySymbol(request.assetSymbol()).orElseThrow(() -> new AssetNotFoundException("Asset not found"));

        WalletBalance walletBalance = walletBalanceRepository
                        .findByWalletIdAndAssetIdForUpdate(wallet.getId(),asset.getId())
                        .orElseThrow(() ->new WalletBalanceNotFoundException("WalletBalance not found"));

        walletBalance.withdraw(request.amount());

        Withdrawal withdrawal = new Withdrawal(request.amount(),wallet,asset,idempotencyKey);
        withdrawal.complete();
